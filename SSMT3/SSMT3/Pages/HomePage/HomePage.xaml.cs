...
namespace SSMT3
{
    ...
    public sealed partial class HomePage : Page
    {
        ...
        private async void GameNameChanged(string ChangeToGameName)
        {
            ...
            CheckBox_DllPreProcess.IsChecked = gameConfig.DllPreProcessSelectedIndex == 1;
   ...
   //             Border_GINoError.Visibility = Visibility.Collapsed;
            ...
            {
                Expander_WWMIConfig.Visibility = Visibility.Visible;
            }
            else
            {
                Expander_WWMIConfig.Visibility = Visibility.Collapsed;
            }
			...
			//最后如果有d3dx.ini的话，如果哪些路径配置还是空的，就试图从d3dx.ini中解析读取，保底机制
            ...
            TextBox_ExtraInjectDll.Text = gameConfig.ExtraInjectDllPath;
            ...
            //{
            //    _ = SSMTMessageHelper.ShowConfirm(SSMTErrorInfo.NotSupportedGamePreset(), "", "我已严肃阅读并知晓",false);
        ...
        private void InitializeToggleConfig()
        {
            ...
            if (File.Exists(d3dxiniPath))
            {
                ...
                {
                    CheckBox_ShowWarning.IsChecked = true;
                }
                else if (ShowWarningsStr.Trim() == "0")
                {
                    CheckBox_ShowWarning.IsChecked = false;
                }
                else
                {
                    CheckBox_ShowWarning.IsChecked = true;
            ...
            CheckBox_ShowIcon.IsChecked = ShowIcon;
        ...
        public async void InstallBasicDllFileTo3DmigotoFolder()
        {
            ...
            string MigotoSourceDll = Path.Combine(PathManager.Path_ResourcesFolder, "ReleaseX64Dev\\d3d11.dll");
            ...
            string MigotoSource47Dll = Path.Combine(PathManager.Path_ResourcesFolder, "d3dcompiler_47.dll");
            string MigotoTarget47Dll = Path.Combine(CurrentGame3DmigotoFolder, "d3dcompiler_47.dll");
            string MigotoSource46Dll = Path.Combine(PathManager.Path_ResourcesFolder, "d3dcompiler_46.dll");
        ...
        private void Button_CreateNewGame_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                CheckBox_ShowIcon.IsChecked = true;
        ...
        private void CheckBox_ShowIcon_Checked(object sender, RoutedEventArgs e)
        {
            if (IsLoading)
            {
                return;
            ...
            GameIconConfig gameIconConfig = new GameIconConfig();
            gameIconConfig.GameName_Show_Dict[GlobalConfig.CurrentGameName] = CheckBox_ShowIcon.IsChecked == true;
        ...
        private void CheckBox_DllPreProcess_Checked(object sender, RoutedEventArgs e)
        {
            if (IsLoading)
            {
                return;
            ...
            GameConfig gameConfig = new GameConfig();
            gameConfig.DllPreProcessSelectedIndex = CheckBox_DllPreProcess.IsChecked == true ? 1 : 0;
            gameConfig.SaveConfig();
        }

        private void CheckBox_AutoSetAnalyseOptions_Checked(object sender, RoutedEventArgs e)
        {
            if (IsLoading)
            {
                return;
            ...
            GameConfig gameConfig = new GameConfig
            {
                AutoSetAnalyseOptions = CheckBox_AutoSetAnalyseOptions.IsChecked == true
            };
            gameConfig.SaveConfig();
        }
        ...
        private void CheckBox_ShowWarning_Checked(object sender, RoutedEventArgs e)
        {
            if (IsLoading)
            {
                return;
            ...
            if (CheckBox_ShowWarning.IsChecked == true)
            {
        ...
        private void ComboBox_GamePreset_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            else if (CurrentGamePreset == GamePreset.WuWa)
            {
                ComboBox_LogicName.SelectedItem = LogicName.WuWa;
                ComboBox_GameTypeFolder.SelectedItem = "WuWa";
                ComboBox_MigotoPackage.SelectedItem = MigotoPackageName.WWMIPackage;
            }
            else if (CurrentGamePreset == GamePreset.AEMI)
            {
                ComboBox_LogicName.SelectedItem = LogicName.AEMI;
                ComboBox_GameTypeFolder.SelectedItem = "AEMI";
            ...
            else if (CurrentGamePreset == GamePreset.IdentityVNeoX3)
            {
        ...
        private void CheckBox_RunWithShell_Checked(object sender, RoutedEventArgs e)
        {
            if (IsLoading)
            {
                return;
            ...
            GameConfig gameConfig = new GameConfig();
            gameConfig.RunWithShell = CheckBox_RunWithShell.IsChecked == true;
            gameConfig.SaveConfig();
        }
        ...
        private void NumberBox_Delay_ValueChanged(NumberBox sender, NumberBoxValueChangedEventArgs args)
        {
            ...
            GameConfig gameConfig = new GameConfig();
            gameConfig.Delay = (int)NumberBox_Delay.Value;
            gameConfig.SaveConfig();
        }

        private void CheckBox_PureGameMode_Checked(object sender, RoutedEventArgs e)
        {
            if (IsLoading)
            {
                return;
            ...
            GameConfig gameConfig = new GameConfig();
            gameConfig.PureGameMode = CheckBox_PureGameMode.IsChecked == true;
            gameConfig.SaveConfig();
        }

        private void Button_OpenProcessFileFolder_Click(object sender, RoutedEventArgs e)
        {
            string targetPath = TextBox_TargetPath.Text.Trim();
            ...
            try
            {
                ...
                if (File.Exists(targetPath))
                {
                    ...
                    {
                        SSMTCommandHelper.ShellOpenFolder(folderPath);
                    }
                }
                // If it's already a directory path
                else if (Directory.Exists(targetPath))
                {
                    SSMTCommandHelper.ShellOpenFolder(targetPath);
                }
                else
                {
                    // Try to open the directory part if the file doesn't exist but the folder might
                    string folderPath = Path.GetDirectoryName(targetPath);
                    ...
                    {
                        _ = SSMTMessageHelper.Show("目标路径不存在。", "Target path does not exist.");
                    }
                }
            }
            catch (Exception ex)
            {
                _ = SSMTMessageHelper.Show("无法打开文件夹: " + ex.Message, "Cannot open folder: " + ex.Message);
            }
        }

        private void Button_OpenLaunchFileFolder_Click(object sender, RoutedEventArgs e)
        {
            string launchPath = TextBox_LaunchPath.Text.Trim();
            ...
            try
            {
                ...
                {
                    string folderPath = Path.GetDirectoryName(launchPath);
                    if (Directory.Exists(folderPath))
                    {
                        SSMTCommandHelper.ShellOpenFolder(folderPath);
                    }
                ...
                {
                    SSMTCommandHelper.ShellOpenFolder(launchPath);
                }
                else
                {
                    // Try to open the directory part if the file doesn't exist but the folder might
                    ...
                    {
                        _ = SSMTMessageHelper.Show("启动路径不存在。", "Launch path does not exist.");
                    }
                }
            }
            catch (Exception ex)
        ...
        private void SaveInjectDllPathToConfig()
        {
            GameConfig gameConfig = new GameConfig();
            gameConfig.ExtraInjectDllPath = TextBox_ExtraInjectDll.Text.Trim();
            gameConfig.SaveConfig();
        }

        private void TextBox_ExtraInjectDll_LostFocus(object sender, RoutedEventArgs e)
        {
            SaveInjectDllPathToConfig();
        }

        private async void Button_ChooseExtraInjectDll_Click(object sender, RoutedEventArgs e)
        {
            string filepath = await SSMTCommandHelper.ChooseFileAndGetPath(".dll");
            if (string.IsNullOrEmpty(filepath))
            {
                return;
            }

            TextBox_ExtraInjectDll.Text = filepath;
            SaveInjectDllPathToConfig();
        }

        private void CheckBox_OverwriteWWMIEngineSetting_Checked(object sender, RoutedEventArgs e)
        {
            ...
            GameConfig gameConfig = new GameConfig();
            gameConfig.OverwriteWWMIEngineSetting = CheckBox_OverwriteWWMIEngineSetting.IsChecked == true;
            gameConfig.SaveConfig();
