...
namespace SSMT3
{
    ...
    public sealed partial class TexturePage : Page
    {
        ...
        private ObservableCollection<string> ComponentNameList = new ObservableCollection<string>();

        public TexturePage()
        {
            this.InitializeComponent();

            //设置ComponentName的列表来源，后续只需要操作列表即可
            ComboBox_Component.ItemsSource = ComponentNameList;

            GameConfig gameConfig = new GameConfig();

            InitializeMenuListBox(gameConfig.GamePreset);
        ...
        private void InitializeMenuListBox(string gamePreset)
        {
            MenuListBox.Items.Clear();
            List<string> menuItems;

            if (gamePreset == GamePreset.ZZMI)
            {
            ...
            else if (gamePreset == GamePreset.GIMI)
            {
                menuItems = new List<string>
                {
                    "DiffuseMap",
            ...
            foreach (var item in menuItems)
            {
                ...
                var listBoxItem = new ListBoxItem { Tag = item, Content = textBlock };
        ...
        private void ComboBoxDrawIB_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (ComboBoxDrawIB.SelectedIndex < 0)
            {
                return;
            }
            ...
            {
                if (texturePageIndexConfig.ComponentIndex < ComponentNameList.Count)
                {
                    ComboBox_Component.SelectedIndex = texturePageIndexConfig.ComponentIndex;
                }
                else
                {
                    ComboBox_Component.SelectedIndex = 0;
        ...
        private void ComboBoxComponent_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (ComboBox_Component.SelectedIndex < 0)
            {
                return;
            }
            
            string DrawIB = ComboBoxDrawIB.SelectedItem.ToString();
            string ComponentName = ComboBox_Component.SelectedItem.ToString();
        ...
        private void ComboBoxDrawCall_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            texturePageIndexConfig.DrawIBIndex = ComboBoxDrawIB.SelectedIndex;
            texturePageIndexConfig.ComponentIndex = ComboBox_Component.SelectedIndex;
        ...
        private void ApplyToAutoTexture()
        {
            ...
            markInfoItem.DrawIB = ComboBoxDrawIB.SelectedItem.ToString();
            markInfoItem.CompoenntName = ComboBox_Component.SelectedItem.ToString();
            markInfoItem.PSHashValue = PSHashValue;
        ...
        private void Menu_GenerateHashStyleTextureModTemplate_Click(object sender, RoutedEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                _ = SSMTMessageHelper.Show("生成贴图Mod失败:\n" + ex.ToString());
            }
